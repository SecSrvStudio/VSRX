...
let scriptHubButton;
let robloxOutputChannel;
let logBuffer = null;
let logBufferTimeout = null;
...
function activate(context) {
    ...
    server.showUIOnLoad = config.get('showUIOnLoad') === true;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vsrx.showScriptHub', async () => {
        await showScriptHub();
    }));
    ...
    if (config.get('showHubButton'))
        scriptHubButton.show();
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('vsrx.showSaveButton')) {
            vscode.workspace.getConfiguration('vsrx').get('showSaveButton') ? savedScriptsButton.show() : savedScriptsButton.hide();
        }
        if (e.affectsConfiguration('vsrx.showHubButton')) {
            vscode.workspace.getConfiguration('vsrx').get('showHubButton') ? scriptHubButton.show() : scriptHubButton.hide();
        }
        if (e.affectsConfiguration('vsrx.enableConsoleCapture')) {
            const enabled = vscode.workspace.getConfiguration('vsrx').get('enableConsoleCapture') !== false;
        ...
        if (e.affectsConfiguration('vsrx.showUIOnLoad')) {
            server.showUIOnLoad = vscode.workspace.getConfiguration('vsrx').get('showUIOnLoad') === true;
        }
...
async function showScriptHub(query = '', page = 1) {
...
function queueLog(message, type, playerName) {
...
function flushLogBuffer() {
    if (!logBuffer || !robloxOutputChannel)
        return;
    if (logBufferTimeout) {
        clearTimeout(logBufferTimeout);
